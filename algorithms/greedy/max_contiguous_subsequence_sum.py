...
def max_contiguous_subsequence_sum(arr) -> int:
    ...
    # Max till now
    max_till_now = arr[0]
    current_subseq_sum = 0 # Set this to 1 to inject fault

    # For every element in array => get max sum of current subseq
    for i in range(0, arr_size):
        if current_subseq_sum + arr[i] < arr[i]: # set this to > to inject fault
            current_subseq_sum = arr[i]
        else:
            current_subseq_sum += arr[i]

        max_till_now = max(max_till_now, current_subseq_sum)
